...
MT="${2:-$METOOLKIT_DIR}"
INC="$MT/include"
SCENE="$KD_T_STD/scene_ragdoll_lod.c"
...
"$KD_T_STD/make_substituted_metoolkit.sh" "$REC" "$MT" "$W/sub" MdtLOD >/dev/null

build() {  # $1 = lib dir, $2 = output binary
    gcc -m32 -O2 -DLINUX -no-pie $IF -o "$2" "$SCENE" \
        -Wl,--start-group "$1"/*.a -Wl,--end-group -lstdc++ -lm 2>&1 | tail -3
}
build "$MT/$KD_MT_LIB_SUBDIR"      "$W/stock" || exit 1
build "$W/sub/$KD_MT_LIB_SUBDIR"   "$W/rec"   || exit 1
