...
OBJDIR="${KD_OBJDIR:-$(dirname "$(find "$UT2004_BUILD_DIR" -name McdGjkPenetrationDepth.c.o -print -quit 2>/dev/null)")}"
SHIPDIR="$KD_OBJ_DIR"
SRCDIR="$KD_ROOT/generated/allobj"
...
if [ "${KD_SCENARIO:-}" = "ragdoll" ]; then
    KD_FRAMES="${KD_RAG_FRAMES:-3000}" KD_EVERY=1 KD_BONES=1 KD_GAME=Onslaught.ONSOnslaughtGame \
        ...
        "$KD_T_UT/ktrace_run.sh" "$BIN" "$LABEL" 300 > "/tmp/ktrace-subst-$LABEL.run.log" 2>&1
else
    KD_FRAMES="$FRAMES" KD_GAME=Onslaught.ONSOnslaughtGame \
        "$KD_T_UT/ktrace_run.sh" "$BIN" "$LABEL" 120 > "/tmp/ktrace-subst-$LABEL.run.log" 2>&1
fi
...
if [ "${KD_SCENARIO:-}" = "ragdoll" ]; then
    python3 "$KD_T_STD/ragdoll_score.py" "${KD_RAG_REF:-/tmp/rd-L.csv}" "/tmp/ktrace-$LABEL.csv"
else
    python3 "$KD_T_STD/ktrace_score.py" "$REF" "/tmp/ktrace-$LABEL.csv"
fi
