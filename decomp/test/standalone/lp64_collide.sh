...
IF="$IF -I$INC/McdFrame -I$INC/Mst -I$INC/MeGlobals -I$INC/MeApp -I$KD_MD_INC"
...
for bits in 32 64; do
    ...
    gcc -m$bits $CF $FPMATH -c -o "$W/o$bits/hull.o" \
        "$KD_ROOT/src/McdConvexCreateHull/kd_convexhull.c" 2>/dev/null || exit 1
    gcc -m$bits $CF $FPMATH -no-pie -o "$W/probe$bits" \
        "$KD_T_STD/lp64_collide.c" "$W/o$bits"/*.o -lstdc++ -lm 2>"$W/link$bits.err" \
