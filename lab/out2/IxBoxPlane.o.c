...

{
  ...
  plVar8 = footprint;
  inRBox = (lsVec3 *)McdBoxGetRadii(uVar6);
  ...
  verts = plVar8;
  if (fVar1 <= (float)(lVar13 + (longdouble)(float)lVar12)) {
...

{
  ...
  uVar7 = McdModelGetTransformPtr(p->model2);
  pfVar8 = (float *)McdModelGetLinearVelocityPtr(p->model1);
  pfVar9 = (float *)McdModelGetLinearVelocityPtr(p->model2);
  fVar2 = *pfVar8 - *pfVar9;
  fVar5 = pfVar8[1] - pfVar9[1];
  fVar4 = pfVar8[2] - pfVar9[2];
  fVar3 = SQRT(fVar2 * fVar2 + fVar5 * fVar5 + fVar4 * fVar4) * maxTime;
  uVar1 = (ushort)(fVar3 < 0.0001) << 8 | (ushort)NAN(fVar3) << 10;
  uVar11 = uVar1 | (ushort)(fVar3 == 0.0001) << 0xe;
  if (uVar1 == 0) {
    uVar10 = McdModelGetTransformPtr(p->model2,uVar11,&planeN,uVar11);
    McdPlaneGetNormal(uVar6,uVar10);
    planeN.v[0] = planeN.v[0] * -1.0;
    planeN.v[1] = planeN.v[1] * -1.0;
    planeN.v[2] = planeN.v[2] * -1.0;
    uVar10 = McdModelGetGeometryInstance(p->model1,&planeN,&closestPoint);
    McdBoxMaximumPoint(uVar10);
    lVar12 = (longdouble)McdPlaneGetDistanceToPoint(uVar6,uVar7,&closestPoint);
    result->time = (float)(lVar12 / ((longdouble)fVar4 * (longdouble)planeN.v[2] +
                                    (longdouble)fVar5 * (longdouble)planeN.v[1] +
                                    (longdouble)fVar2 * (longdouble)planeN.v[0]));
  }
  else {
    result->time = maxTime;
  }
