 ...
 * source object: /home/ion/tools/karma-lab/allobj/IxCylinderCylinder.o
 */
#include "kd_compat.h"
#include "kd_karma.h"
#include "kd_types.h"
...
/*  Prelude for IxCylinderCylinder.o — generated by tools/gen_prelude.py.
 *
...
/* FnMinNd(int, float*, float, float, float (*)(float*), void (*)(float*, float*)) */
extern float FnMinNd(int, void *, float, float, void *, void *)
    KD_MANGLED("_Z7FnMinNdiPfffPFfS_EPFvS_S_E");

/* --- file-scope statics --- */
/* dispDotX1  (.data+0x0, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal dispDotX1;

/* dispDotY1  (.data+0x4, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal dispDotY1;

/* dispDotX2  (.data+0x8, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal dispDotX2;

/* dispDotY2  (.data+0xc, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal dispDotY2;

/* X1DotX2  (.data+0x10, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal X1DotX2;

/* X1DotY2  (.data+0x14, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal X1DotY2;

/* Y1DotX2  (.data+0x18, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal Y1DotX2;

/* Y1DotY2  (.data+0x1c, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal Y1DotY2;

/* cylR1  (.data+0x20, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal cylR1;

/* cylR2  (.data+0x24, 4 bytes) */
/* type from this object's DWARF; bytes are all zero */
static MeReal cylR2;

/* ---- forward declarations ---- */
void kd_CylCylIntersect(lsVec3 **outList,lsTransform *tAB,MeReal inR1,MeReal inHH1,lsVec3 *perp1, lsVec3 *para1,bool doCyl1,MeReal inR2,MeReal inHH2,lsVec3 *perp2,lsVec3 *para2, bool doCyl2,MeReal scale) KD_MANGLED("_Z15CylCylIntersectRP6lsVec3RK11lsTransformffRKS_S6_bffS6_S6_bf");
MeReal kd_EndCapMin(MeReal *theta) KD_MANGLED("_Z9EndCapMinPf");
void kd_EndCapMinGrad(MeReal *grad,MeReal *theta) KD_MANGLED("_Z13EndCapMinGradPfS_");
bool kd_OverlapCylCyl(MeReal *outSep,lsVec3 *outN,MeReal *outPN,lsVec3 **outPos,MeI16 *outDims, MeReal inEps,MeReal inR1,MeReal inHH1,MeReal inR2,MeReal inHH2,lsTransform *inT12, MeReal scale) KD_MANGLED("_Z13OverlapCylCylRfR6lsVec3S_RPS0_RsfffffRK11lsTransformf");
MeBool kd_McdCylinderCylinderIntersect(McdModelPair *p,McdIntersectResult *result) KD_MANGLED("McdCylinderCylinderIntersect");
int kd_McdCylinderCylinderSafeTime(McdModelPair *p,MeReal maxTime,McdSafeTimeResult *result) KD_MANGLED("McdCylinderCylinderSafeTime");
MeBool kd_McdCylinderCylinderRegisterInteraction(McdFramework *frame) KD_MANGLED("McdCylinderCylinderRegisterInteraction");
...
void kd_CylPerpAndPara(lsVec3 *perp,lsVec3 *para,lsVec3 *axis,lsVec3 *n,lsVec3 *disp) KD_MANGLED("_Z14CylPerpAndParaR6lsVec3S0_RKS_S2_S2_") KD_WEAK;
...
/* ---- CylCylIntersect (exported as kd_CylCylIntersect, asm label "kd_CylCylIntersect") ---- */
void kd_CylCylIntersect(lsVec3 **outList,lsTransform *tAB,MeReal inR1,MeReal inHH1,lsVec3 *perp1,
...
/* ---- EndCapMin (exported as kd_EndCapMin, asm label "kd_EndCapMin") ---- */
MeReal kd_EndCapMin(MeReal *theta)
...
/* ---- EndCapMinGrad (exported as kd_EndCapMinGrad, asm label "kd_EndCapMinGrad") ---- */
void kd_EndCapMinGrad(MeReal *grad,MeReal *theta)
...
/* ---- OverlapCylCyl (exported as kd_OverlapCylCyl, asm label "kd_OverlapCylCyl") ---- */
bool kd_OverlapCylCyl(MeReal *outSep,lsVec3 *outN,MeReal *outPN,lsVec3 **outPos,MeI16 *outDims,
...

{
  ...
  if (fVar3 <= inEps) {
    ...
    if ((((ABS(fVar5) <= ABS(nPara1.v[2])) &&
      ...
      theta[1] = 0.0;
      theta[0] = 0.0;
      FnMinNd(2,theta,0.001,0.001,kd_EndCapMin,kd_EndCapMinGrad);
      ...
      if ((normInfo & 3U) == 2) {
        if (cVar10 == '\x02') {
          ...
          *outDims = 0x303;
          ...
          vCross_3.v[1] = 0.0;
          vCross_3.v[2] = 1.0;
          kd_CylPerpAndPara(&nPerp1,&nPara1,&vCross_3,(lsVec3 *)n,(lsVec3 *)(inT12->row + 3));
        ...
        if (cVar10 == '\0') {
          ...
          kd_CylPerpAndPara(&nPerp2,&nPara2,(lsVec3 *)(inT12->row + 2),&nA2E1,
                         (lsVec3 *)(inT12->row + 3));
        ...
        if ((normInfo & 3U) == 0) {
          ...
          vCross_3.v[1] = 0.0;
          vCross_3.v[2] = 1.0;
          kd_CylPerpAndPara(&nPerp1,&nPara1,&vCross_3,&nA1E2,(lsVec3 *)(inT12->row + 3));
...
LAB_000133e8:
  ...
  vCross_3.v[1] = 0.0;
  vCross_3.v[0] = 0.0;
  kd_CylPerpAndPara(plVar25,para,(lsVec3 *)n,(lsVec3 *)n_00,(lsVec3 *)(inT12->row + 3));
LAB_00013034:
  kd_CylCylIntersect(outPos,inT12,inR1,inHH1,&nPerp1,&nPara1,true,inR2,inHH2,&nPerp2,&nPara2,true,scale
                 );
  return true;
}

/* ---- McdCylinderCylinderIntersect (exported as kd_McdCylinderCylinderIntersect, asm label "McdCylinderCylinderIntersect") ---- */
MeBool kd_McdCylinderCylinderIntersect(McdModelPair *p,McdIntersectResult *result)

{
  ...
  plVar10 = footprint;
  verts = plVar10;
  bVar2 = kd_OverlapCylCyl(&separation,&normal,&PN,&verts,&dims,fVar12 + fVar11,inR1,inHH1,inR2,inHH2,
                        &tAB,pMVar1->mScale);
  uVar9 = 0;
  if (bVar2) {
...
/* ---- McdCylinderCylinderSafeTime (exported as kd_McdCylinderCylinderSafeTime, asm label "McdCylinderCylinderSafeTime") ---- */
int kd_McdCylinderCylinderSafeTime(McdModelPair *p,MeReal maxTime,McdSafeTimeResult *result)
...
/* ---- McdCylinderCylinderRegisterInteraction (exported as kd_McdCylinderCylinderRegisterInteraction, asm label "McdCylinderCylinderRegisterInteraction") ---- */
MeBool kd_McdCylinderCylinderRegisterInteraction(McdFramework *frame)

{
  ...
  interactions.intersectFn = kd_McdCylinderCylinderIntersect;
  interactions.safetimeFn = kd_McdCylinderCylinderSafeTime;
  interactions.cull = 1;
...
/* ---- CylPerpAndPara (exported as kd_CylPerpAndPara, asm label "kd_CylPerpAndPara") ---- */
void kd_CylPerpAndPara(lsVec3 *perp,lsVec3 *para,lsVec3 *axis,lsVec3 *n,lsVec3 *disp)
