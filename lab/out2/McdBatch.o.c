...

{
  ...
  MeU16 MVar8;
  int iVar9;
  MeMatrix4Ptr paMVar10;
  int iVar11;
  MeBool MVar12;
  float *pfVar13;
  McdBatchEntry *pMVar14;
  ...
  iVar9 = McdGeometryInstanceGetGeometry(ins1);
  pMVar15 = ins1->child;
  iVar16 = 0;
  McdGeometryInstanceGetAABB(ins2,min2,max2);
  MVar8 = McdGeometryInstanceGetGeometryType(ins2);
  iVar11 = *(int *)(iVar9 + 0x18);
  if (0 < iVar11) {
    local_70 = 0;
    do {
      iVar6 = *(int *)(*(int *)(iVar9 + 0x10) + 0x40 + local_70);
      if (iVar6 != 0) {
        McdGeometryInstanceGetAABB(pMVar15,min1,max1,iVar6);
        if ((((min1[0] <= max2[0]) && (min1[1] <= max2[1])) && (min1[2] <= max2[2])) &&
          ...
          if (pMVar15->mTM == (MeMatrix4Ptr)0x0) {
            ...
            paMVar10 = pMVar15->mTM;
            paMVar7 = ins1->mTM;
            pfVar13 = (float *)(local_70 + *(int *)(iVar9 + 0x10));
            i = 3;
            do {
              (*paMVar10)[0] =
                   paMVar7[3][0] * pfVar13[3] +
                   paMVar7[2][0] * pfVar13[2] +
                   paMVar7[1][0] * pfVar13[1] + (*paMVar7)[0] * *pfVar13;
              (*paMVar10)[1] =
                   paMVar7[3][1] * pfVar13[3] +
                   paMVar7[2][1] * pfVar13[2] +
                   paMVar7[1][1] * pfVar13[1] + (*paMVar7)[1] * *pfVar13;
              (*paMVar10)[2] =
                   paMVar7[3][2] * pfVar13[3] +
                   paMVar7[2][2] * pfVar13[2] +
                   paMVar7[1][2] * pfVar13[1] + (*paMVar7)[2] * *pfVar13;
              fVar4 = *pfVar13;
              pfVar1 = pfVar13 + 1;
              pfVar2 = pfVar13 + 2;
              pfVar3 = pfVar13 + 3;
              ...
              (*paMVar10)[3] =
                   paMVar7[3][3] * *pfVar3 +
                   paMVar7[2][3] * *pfVar2 + paMVar7[1][3] * *pfVar1 + (*paMVar7)[3] * fVar4;
              paMVar10 = paMVar10 + 1;
              i = i + -1;
            } while (-1 < i);
          }
          bVar5 = **(byte **)(*(int *)(iVar9 + 0x10) + 0x40 + local_70);
          flags_00 = flags;
          ins1_00 = pMVar15;
          ins2_00 = ins2;
          eps1_00 = eps1;
          ...
          if (bVar5 != 8) {
            if ((short)MVar8 != 8) {
              ...
              iVar11 = context->nextEntry;
              if (context->maxEntryCount <= iVar11) {
                return 0;
              }
              pMVar14 = context->entryArray + iVar11;
              iVar11 = McdFrameworkGetInteractions(context->frame,bVar5,(int)(short)MVar8,iVar11);
              if (*(int *)(iVar11 + 0x10) == 0) {
                (pMVar14->geometryData1).type = (ushort)bVar5;
                (pMVar14->geometryData1).min = pMVar15->min;
                (pMVar14->geometryData1).max = pMVar15->max;
                (pMVar14->geometryData1).eps = eps1;
                (pMVar14->geometryData1).geometry = pMVar15->mGeometry;
                (pMVar14->geometryData1).tm = pMVar15->mTM;
                (pMVar14->geometryData2).type = MVar8;
                (pMVar14->geometryData2).eps = eps2;
                (pMVar14->geometryData2).min = ins2->min;
                (pMVar14->geometryData2).max = ins2->max;
                (pMVar14->geometryData2).geometry = ins2->mGeometry;
                (pMVar14->geometryData2).tm = ins2->mTM;
                pMVar14->ins1 = pMVar15;
                pMVar14->ins2 = ins2;
                pMVar14->flags = flags | 2;
              }
              else {
                (pMVar14->geometryData1).type = MVar8;
                (pMVar14->geometryData1).eps = eps2;
                (pMVar14->geometryData1).min = ins2->min;
                (pMVar14->geometryData1).max = ins2->max;
                (pMVar14->geometryData1).geometry = ins2->mGeometry;
                (pMVar14->geometryData1).tm = ins2->mTM;
                (pMVar14->geometryData2).type = (ushort)bVar5;
                (pMVar14->geometryData2).eps = eps1;
                (pMVar14->geometryData2).min = pMVar15->min;
                (pMVar14->geometryData2).max = pMVar15->max;
                (pMVar14->geometryData2).geometry = pMVar15->mGeometry;
                (pMVar14->geometryData2).tm = pMVar15->mTM;
                pMVar14->ins2 = pMVar15;
                pMVar14->ins1 = ins2;
                pMVar14->flags = (flags >> 1 & 2U | flags | 4) ^ 1;
              }
              iVar11 = (uint)(pMVar14->geometryData1).type * context->typeCount +
                       (uint)(pMVar14->geometryData2).type;
              pMVar14->next = context->table[iVar11];
              context->table[iVar11] = pMVar14;
          ...
          MVar12 = McdBatchFlattenAggregate(context,flags_00,ins1_00,ins2_00,eps1_00,eps2_00);
          if (MVar12 == 0) {
            return 0;
          }
        }
  ...
  if (context->nextFlattenPair < pairs->stayingEnd) {
    do {
      ...
      if ((iVar10 == 0) || (iVar10 = McdSpaceModelIsFrozen(pMVar7), iVar10 == 0)) {
        ins1_00 = (McdGeometryInstanceID)McdModelGetGeometryInstance(pMVar6);
        ins2_00 = (McdGeometryInstanceID)McdModelGetGeometryInstance(pMVar7);
        bVar3 = (byte)ins1_00->mGeometry->mRefCtAndID;
        bVar4 = (byte)ins2_00->mGeometry->mRefCtAndID;
        if (bVar3 == 8) {
          lVar15 = (longdouble)McdModelGetContactTolerance(pMVar6);
          lVar16 = (longdouble)McdModelGetContactTolerance(pMVar7);
          iVar10 = context->nextEntry;
          iVar11 = context->nextFlattenPair;
          iVar12 = context->nextTM;
          pMVar1->start = context->entryArray + iVar10;
          MVar8 = McdBatchFlattenAggregate(context,0,ins1_00,ins2_00,(float)lVar15,(float)lVar16);
          if (MVar8 == 0) {
...
LAB_00010772:
          ...
          (pMVar13->geometryData1).eps = (float)lVar15;
          ...
          lVar15 = (longdouble)McdModelGetContactTolerance(pMVar7);
          (pMVar13->geometryData2).type = (ushort)bVar4;
          (pMVar13->geometryData2).eps = (float)lVar15;
...

{
  ...
  iVar7 = type1 * context->typeCount + type2;
  iVar5 = McdFrameworkGetInteractions(context->frame,type1,type2);
  pcVar2 = *(code **)(iVar5 + 8);
  if ((pcVar2 == (code *)0x0) &&
     (iVar5 = McdFrameworkGetInteractionsWarned(context->frame,type1,type2,iVar5), iVar5 == 0)) {
    uVar6 = McdFrameworkGetTypeName(context->frame,type2,0,0);
    uVar6 = McdFrameworkGetTypeName(context->frame,type1,uVar6,uVar6);
    MeWarning(1,"No test registered between %s and %s",uVar6);
    McdFrameworkSetInteractionsWarned(context->frame,type1,type2,1);
  }
  pMVar3 = context->table[iVar7];
  while( true ) {
    ...
    r.pair = &p;
    iVar5 = pMVar1->contactCount;
    r.contactMaxCount = pMVar1->contactMaxCount - iVar5;
    if (pcVar2 == (code *)0x0) {
      r.touch = 0;
      r.contactCount = 0;
    }
    ...
    context->table[iVar7] = pMVar3->next;
    pMVar3 = context->table[iVar7];
  }
  return 0;
}
...

{
  ...
  iVar6 = context->nextSingleUnflattenPair;
  iVar7 = context->nextPairData;
  if (iVar6 < iVar7) {
    do {
      pMVar1 = context->pairData + iVar6;
      if (pMVar1->status == 2) {
        ...
        *resultCount = iVar6 + 1;
        ...
        if (pMVar3->touch != 0) {
          ...
          MVar5 = pMVar3->normal[2];
          pMVar2->normal[2] = MVar5;
          iVar6 = McdFrameworkGetInteractions
                            (context->frame,(pMVar3->geometryData1).type,
                             (pMVar3->geometryData2).type,MVar5);
          if (*(int *)(iVar6 + 0x14) == 0) {
            iVar6 = pMVar3->contactCount;
            pMVar2->contactCount = iVar6;
            pMVar8 = pMVar3->contacts;
            pMVar9 = pMVar2->contacts;
            for (iVar6 = (iVar6 * 5 & 0x1fffffffU) << 1; iVar6 != 0; iVar6 = iVar6 + -1) {
            ...
            iVar6 = McdContactSimplify(pMVar2->normal,pMVar3->contacts,pMVar3->contactCount,
      ...
      iVar6 = context->nextSingleUnflattenPair + 1;
      context->nextSingleUnflattenPair = iVar6;
    } while (iVar6 < iVar7);
  }
  iVar6 = context->nextAggregateUnflattenPair;
  if (iVar6 < iVar7) {
    do {
                    /* Unresolved local var: McdBatchPairData * pd@[DW_OP_reg6(ESI)] */
      pMVar1 = context->pairData + iVar6;
      ...
      iVar6 = context->nextAggregateUnflattenPair + 1;
      context->nextAggregateUnflattenPair = iVar6;
    } while (iVar6 < iVar7);
  }
  return 1;
}
