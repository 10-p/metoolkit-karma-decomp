...
void MdtAutoDisableLastPartition(MdtPartitionOutput *po,void *pcbdata)
...

{
  ...
  do {
    ...
    if (0 < (int)uVar7) {
      ...
      if ((int)uVar7 < 2) {
LAB_00010442:
        MdtUpdateBodyForces(ppMVar10[i],stepSize,params->gravity);
        ppMVar2 = ppMVar10 + i;
        i = i + 1;
        ...
        if (1 < uVar6) {
          if (2 < uVar6) {
            MdtUpdateBodyForces(*ppMVar10,stepSize,params->gravity);
      ...
      do {
        MdtUpdateBodyForces(ppMVar10[i],stepSize,params->gravity);
        GetCOMTransform(ppMVar10[i],local_50);
        MdtUpdateBodyForces(ppMVar10[i + 1],stepSize,params->gravity);
        GetCOMTransform(ppMVar10[i + 1],local_50 + 1);
        MdtUpdateBodyForces(ppMVar10[i + 2],stepSize,params->gravity);
        GetCOMTransform(ppMVar10[i + 2],local_50 + 2);
        MdtUpdateBodyForces(ppMVar10[i + 3],stepSize,params->gravity);
...
void MdtUnpackBodies(MdtKeaTransformation *keaTMArray,uint partitionindex,MdtPartitionOutput *po)
