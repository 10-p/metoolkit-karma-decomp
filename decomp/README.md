...
- **The precise gate is general now.** `difftest_pair.sh` runs it for any interaction, not one —
  `difftest_pair.sh <build> <metoolkit> [pair] [iters]`. The single-function `difftest_boxbox.c`
  that first proved the method has been removed; it did nothing `difftest_pair.c` does not.
...
```
HANDOVER.md                          the working document — read this first
HANDOVER-WEB.md                      the wasm/Android integration brief
STATUS-EXEC.md                       one page, plain language
proven.txt                           the evidence ledger; a line here RELEASES an object

tools/README.md                      every generator and analyser, with the command
tools/gscripts/DumpDecomp.java       Ghidra headless decompile + per-function stats + <obj>.locals
tools/gscripts/ParseKarmaHeaders.java  preScript: applies kd_protos11.h
tools/ghidra_clean.py                Ghidra dump -> compilable C (the pre-passes + the repair loop)
...
The recovered sources are **generated output**. Every fix goes into `tools/`, and the acceptance
test is that every already-compiling object's `.o` stays byte-identical.
...
**This section dates from the first sessions and most of it has been closed.** `HANDOVER.md` §12 is
the current, maintained answer to "what is left"; what follows is kept only where it still holds.
...
- ~~`writeKeaInputToFile` / `readKeaInputFromFile` — stub them~~ — **superseded.**
  `ReadWriteKeaInputToFile` is recovered and in the build. `MeViewer2` and `MeApp` remain out of
  scope, and `tools/reachable.py` now proves it rather than asserting it: 31 members are unreachable
  from the engine, and every one is `ar d`'d out of the drop-in tree that links and plays.
