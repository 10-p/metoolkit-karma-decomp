...

{
  ...
  if ((j->head).mdtbody[1] == (MdtBody *)0x0) {
    ...
    ConvertCOMVector(0,direction,(j->head).mdtbody[1],j->direction);
  ...
  if (pMVar1 == (MdtBody *)0x0) {
    ...
    ConvertCOMVector(0,vector1,(j->head).mdtbody[1],j->vec1);
    ConvertCOMVector(0,vector2,(j->head).mdtbody[1],j->vec2);
  }
...

{
  ...
  pMVar1 = (MdtLinear2ID)BaseConstraintCreate(w);
  pMVar3 = (MdtLinear2ID)0x0;
  if (pMVar1 != (MdtLinear2ID)0x0) {
                    /* Unresolved local var: int i@[???] */
    (pMVar1->head).tag = 0xb;
    (pMVar1->head).bclFunction = (MdtBclAddConstraintFn)&MdtBclAddLinear2;
...
void __regparm1 MdtLinear2GetPosition(MdtLinear2ID j,MeReal *position)
...
void __regparm1 MdtLinear2GetDirection(MdtLinear2ID j,MeReal *direction)

{
  MeReal *in_stack_00000008;
  
  if (direction[0x16] == 0.0) {
    *in_stack_00000008 = direction[0x5e];
    in_stack_00000008[1] = direction[0x5f];
    in_stack_00000008[2] = direction[0x60];
  }
  else {
    ConvertCOMVector(direction[0x16],direction + 0x5e,0);
  }
  return;
}
...

{
  ...
  j->pos2[2] = z;
  j->pos2[1] = y;
  ConvertCOMPositionVector(0,pMVar1,(j->head).mdtbody[0],j->pos1);
  pMVar2 = (j->head).mdtbody[1];
  if (pMVar2 != (MdtBody *)0x0) {
    ConvertCOMPositionVector(0,pMVar1,pMVar2,pMVar1);
  }
