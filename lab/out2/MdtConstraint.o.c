...

{
  ...
  (c->head).ref2[3][2] = 0.0;
  (c->head).ref2[3][3] = 1.0;
  MeDictNodeInit(&(c->head).worldNode,(c->head).sortKey,paMVar1,paMVar1);
  (c->head).worldNode.data = c;
  MeDictNodeInit((c->head).bodyNode,(c->head).sortKey);
  (c->head).bodyNode[0].data = c;
  MeDictNodeInit((c->head).bodyNode + 1,(c->head).sortKey);
...

{
  ...
  if ((uVar3 & 1) != 0) {
    ...
    (c->head).flags = uVar3;
    MeDictDelete(&((c->head).world)->constraintDict,&(c->head).worldNode,uVar3,uVar3);
    pMVar2 = (c->head).world;
    ...
    if (pMVar1 != (MdtBody *)0x0) {
      MeDictDelete(&pMVar1->constraintDict,unaff_ESI,extraout_ECX,extraout_ECX);
      uVar4 = extraout_ECX_00;
    }
    pMVar1 = (c->head).mdtbody[1];
    if (pMVar1 != (MdtBody *)0x0) {
      MeDictDelete(&pMVar1->constraintDict,(c->head).bodyNode + 1,uVar4,uVar4);
    }
  }
  if ((c->head).tag == 5) {
    pMVar2 = (c->head).world;
    if (pMVar2->contactGroupDestroyCallback == (MdtContactGroupDestroyCallbackCBPtr)0x0)
    goto LAB_0001089d;
    (*pMVar2->contactGroupDestroyCallback)((MdtContactGroupID_conflict)c);
  }
  pMVar2 = (c->head).world;
LAB_0001089d:
  (c->head).tag = -1;
  (*_ConvertCOMPositionVector)(&pMVar2->constraintPool,c,unaff_ESI,unaff_ESI);
  return;
}
...

{
  ...
  if (((uVar4 & 1) == 0) &&
    ...
    uVar4 = uVar4 | 1;
    (c->head).flags = uVar4;
    MeDictInsert(&pMVar2->constraintDict,&(c->head).worldNode,(c->head).sortKey,uVar4);
    pMVar2->nEnabledConstraints = pMVar2->nEnabledConstraints + 1;
    pMVar1 = (c->head).bodyNode;
    pMVar3 = (c->head).mdtbody[0];
    if (pMVar3 != (MdtBody *)0x0) {
      MeDictInsert(&pMVar3->constraintDict,pMVar1,(c->head).sortKey,extraout_ECX);
    }
    pMVar3 = (c->head).mdtbody[1];
    if (pMVar3 != (MdtBody *)0x0) {
      MeDictInsert(&pMVar3->constraintDict,(c->head).bodyNode + 1,(c->head).sortKey,pMVar1);
    }
  }
  return;
}
...

{
  ...
  if ((uVar3 & 1) != 0) {
    ...
    MeDictDelete(&((c->head).world)->constraintDict,&(c->head).worldNode,uVar3,uVar3);
    pMVar1 = (c->head).world;
    pMVar1->nEnabledConstraints = pMVar1->nEnabledConstraints + -1;
    pMVar2 = (c->head).mdtbody[0];
    if (pMVar2 != (MdtBody *)0x0) {
      MeDictDelete(&pMVar2->constraintDict,(c->head).bodyNode,extraout_ECX,extraout_ECX);
    }
    pMVar2 = (c->head).mdtbody[1];
    if (pMVar2 != (MdtBody *)0x0) {
      MeDictDelete(&pMVar2->constraintDict,(c->head).bodyNode + 1,c,c);
    }
  }
  return;
}
...

{
  ...
  if (iVar1 == 0) {
    iVar2 = (c->head).maxRows;
  }
  else {
                    /* Unresolved local var: MdtContact * contact@[DW_OP_reg0(EAX)] */
    for (iVar1 = *(int *)(iVar1 + 0x164); iVar1 != 0; iVar1 = *(int *)(iVar1 + 0x1dc)) {
...
void __regparm1 MdtConstraintGetPosition(MdtConstraintID c,MeReal *p)

{
  ConvertCOMPositionVector(p[0x16],p + 0x4b,0);
  return;
}
...
void __regparm1 MdtConstraintBodyGetPosition(MdtConstraintID c,uint bodyindex,MeReal *a)

{
  ...
  if (a == (MeReal *)0x0) {
    iVar2 = bodyindex + 0xec;
    uVar1 = *(undefined4 *)(bodyindex + 0x54);
  }
  else {
    iVar2 = bodyindex + 300;
    uVar1 = *(undefined4 *)(bodyindex + 0x58);
  }
  ConvertCOMPositionVector(uVar1,iVar2,0);
  return;
}
...
void __regparm1 MdtConstraintBodyGetAxis(MdtConstraintID c,uint bodyindex,MeReal *a)

{
  ...
  if (a == (MeReal *)0x0) {
    iVar2 = bodyindex + 0xbc;
    uVar1 = *(undefined4 *)(bodyindex + 0x54);
  }
  else {
    iVar2 = bodyindex + 0xfc;
    uVar1 = *(undefined4 *)(bodyindex + 0x58);
  }
  ConvertCOMVector(uVar1,iVar2,0);
  return;
}
...
{
  if (bodyindex == 0) {
    ConvertCOMVector((c->head).mdtbody[0],(c->head).ref1,0,p);
  }
  else {
    ConvertCOMVector((c->head).mdtbody[1],(c->head).ref2,0,p);
  }
  ConvertCOMVector();
  return;
}
...
void __regparm1 MdtConstraintGetAxes(MdtConstraintID c,MeReal *p,MeReal *o)

{
  undefined4 in_stack_0000000c;
  
  ConvertCOMVector(p[0x16],p + 0x3f,0,o,c);
  ConvertCOMVector(p[0x16],p + 0x43,0,in_stack_0000000c);
  return;
}
...

{
  ...
  iVar1 = MeDictFirst(&w->constraintDict);
  pMVar2 = (MdtConstraintID)0x0;
  if (iVar1 != 0) {
    pMVar2 = *(MdtConstraintID *)(iVar1 + 0x14);
  }
  return pMVar2;
}
...

{
  ...
  MdtConstraintID pMVar2;
                    ...
                    /* Unresolved local var: MeDictNode * node@[DW_OP_reg0(EAX)] */
  iVar1 = MeDictNext(&((c->head).world)->constraintDict,&(c->head).worldNode);
  pMVar2 = (MdtConstraintID)0x0;
  if (iVar1 != 0) {
    pMVar2 = *(MdtConstraintID *)(iVar1 + 0x14);
  }
...

{
  ...
  ConvertCOMPositionVector(0,pos,pMVar2,paMVar1 + 3);
  return;
}
...

{
  ...
  pos_1[1] = y;
  pos_1[2] = z;
  ConvertCOMPositionVector(0,pos_1,(c->head).mdtbody[1],(c->head).ref2 + 3);
  return;
}
