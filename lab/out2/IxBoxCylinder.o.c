...

{
  ...
  pfVar4 = (float *)McdModelGetTransformPtr(p->model1);
  pfVar5 = (float *)McdModelGetTransformPtr(p->model2);
  lVar13 = (longdouble)McdModelGetContactTolerance(p->model1);
  lVar14 = (longdouble)McdModelGetContactTolerance(p->model2);
  uVar6 = McdModelGetGeometry(p->model1);
  uVar7 = McdModelGetGeometry(p->model2);
  pMVar1 = p->model1->frame;
  result->contactCount = 0;
  result->touch = 0;
  inROBB = (lsVec3 *)McdBoxGetRadii(uVar6);
  lVar15 = (longdouble)McdCylinderGetRadius(uVar7);
  lVar16 = (longdouble)McdCylinderGetHalfHeight(uVar7);
  iVar8 = 0x2f;
  ...
  plVar10 = footprint;
  verts = plVar10;
  bVar3 = OverlapOBBCyl(&separation,&normal,&PN,&verts,(float)(lVar14 + (longdouble)(float)lVar13),
                        inROBB,(float)lVar15,(float)lVar16,&tAB,pMVar1->mScale);
  uVar11 = 0;
  if (bVar3) {
    uVar9 = (ushort)(normal.v[0] < 0.0001);
    ...
    dimB = 2;
    fVar2 = normal.v[2] * tAB.row[2].v.v[2] +
            normal.v[1] * tAB.row[2].v.v[1] + normal.v[0] * tAB.row[2].v.v[0];
    fVar2 = fVar2 * fVar2;
    if ((fVar2 <= 0.999999) && (dimB = 3, 1e-06 <= fVar2)) {
      dimB = 1;
    }
    result->normal[0] = normal.v[2] * pfVar4[8] + normal.v[1] * pfVar4[4] + normal.v[0] * *pfVar4;
    result->normal[1] = normal.v[2] * pfVar4[9] + normal.v[0] * pfVar4[1] + normal.v[1] * pfVar4[5];
    result->normal[2] = normal.v[0] * pfVar4[2] + normal.v[1] * pfVar4[6] + normal.v[2] * pfVar4[10]
    ;
    result->contactCount = 0;
    pMVar12 = result->contacts;
    if (plVar10 == verts) {
      iVar8 = result->contactCount;
    }
      ...
      do {
        ...
        fVar2 = (normal.v[2] * plVar10->v[2] +
                plVar10->v[0] * normal.v[0] + normal.v[1] * plVar10->v[1]) - PN;
        if (fVar2 < separation * 0.01) {
          pMVar12->position[0] =
               plVar10->v[0] * *pfVar4 + plVar10->v[1] * pfVar4[4] + plVar10->v[2] * pfVar4[8] +
               pfVar4[0xc];
          pMVar12->position[1] =
               pfVar4[9] * plVar10->v[2] + pfVar4[5] * plVar10->v[1] + pfVar4[1] * plVar10->v[0] +
               pfVar4[0xd];
          pMVar12->position[2] =
               pfVar4[10] * plVar10->v[2] + pfVar4[6] * plVar10->v[1] + pfVar4[2] * plVar10->v[0] +
               pfVar4[0xe];
          pMVar12->separation = fVar2;
          pMVar12->dims = dimB << 2 | uVar9;
          pMVar12->normal[0] = result->normal[0];
          pMVar12->normal[1] = result->normal[1];
          pMVar12->normal[2] = result->normal[2];
        ...
        iVar8 = result->contactCount;
      } while (iVar8 < result->contactMaxCount);
    }
    uVar11 = (uint)(0 < iVar8);
    result->touch = uVar11;
  }
...

{
  ...
  result->pair = p;
  result->time = maxTime;
  uVar1 = McdModelGetGeometry(p->model1);
  uVar2 = McdModelGetGeometry(p->model2);
  plVar3 = (lsVec3 *)McdModelGetLinearVelocityPtr(p->model1);
  plVar4 = (lsVec3 *)McdModelGetLinearVelocityPtr(p->model2);
  lVar9 = (longdouble)McdCylinderGetRadius(uVar2);
  lVar10 = (longdouble)McdCylinderGetHalfHeight(uVar2);
  plVar5 = (lsTransform *)McdModelGetTransformPtr(p->model1);
  plVar6 = (lsTransform *)McdModelGetTransformPtr(p->model2);
  pfVar7 = (float *)McdBoxGetRadii(uVar1);
  ext2[0] = (float)lVar9 * 0.8284271;
  ext2[1] = (float)lVar9 * 0.8284271;
  ext2[2] = (float)lVar10;
  iVar8 = MovingBoxBoxIntersect(pfVar7,plVar5,plVar3,ext2,plVar6,plVar4,maxTime,&T,&P);
  if (iVar8 == 0) {
    result->time = maxTime;
  }
  else {
