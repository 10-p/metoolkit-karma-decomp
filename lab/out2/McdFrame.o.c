...

{
  ...
  pcVar2 = (char *)MeToolkitVersionString();
...

{
  ...
  do {
    ...
    pMVar3 = pMVar4->next;
    iVar6 = McdGeometryGetReferenceCount(pMVar4);
    if (iVar6 == 0) {
      McdGeometryDestroy(pMVar4);
    }
    pMVar2 = f->firstGeometry;
    pMVar4 = pMVar3;
    guard = iVar5;
  } while ((pMVar3 != pMVar2) || (guard = iVar5 + 1, iVar5 < 0x65));
  MeWarning(1,
...

{
  ...
  uVar2 = MeInfo(0,"      ( #types registered: %d )",frame->geometryRegisteredCount);
  if (0 < frame->geometryRegisteredCount) {
    iVar5 = 0;
    iVar3 = 0;
    do {
      puVar1 = (undefined4 *)((int)&frame->geometryVTableTable->name + iVar5);
      iVar5 = iVar5 + 0x28;
      iVar4 = iVar3 + 1;
      uVar2 = MeInfo(0,"   index #%d: %s",iVar3,*puVar1);
      iVar3 = iVar4;
    } while (iVar4 < frame->geometryRegisteredCount);
  }
  MeInfo(0,&DAT_00010760,uVar2,uVar2);
  return;
}
