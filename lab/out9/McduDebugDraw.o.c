...

{
  ...
  pvVar1 = McdModelGetGeometry(model);
  ...
  if ((flags & kMcduDebugDrawDetail) != 0) {
    McdGeometryDebugDraw(pvVar1,pvVar2,colour);
  }
  return;
}
...
LAB_00010290:
  iVar2 = McdSpaceGetModel(space,&it,&m);
  if (iVar2 == 0) {
    return;
  }
...
LAB_000102c5:
    pMVar1 = m;
    pvVar3 = McdModelGetGeometry(m);
    pvVar4 = McdModelGetTransformPtr(pMVar1);
    if ((flags & kMcduDebugDrawAABB) != 0) {
      McdModelGetAABB(pMVar1,min,max);
      McduDebugDrawAABB(min,max,colour);
    }
    if ((flags & kMcduDebugDrawDetail) != 0) {
      McdGeometryDebugDraw(pvVar3,pvVar4,colour);
    }
  }
  goto LAB_00010290;
}
