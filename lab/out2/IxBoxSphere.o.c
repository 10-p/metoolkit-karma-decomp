...

{
  ...
  inROBB = (lsVec3 *)McdBoxGetRadii(uVar5);
  lVar10 = (longdouble)McdSphereGetRadius(uVar6);
  ...
  bVar2 = McdVanillaOverlapOBBSphere
                    (&separation,&normal,&pos,&dims,(float)(lVar9 + (longdouble)(float)lVar8),inROBB
                     ,(float)lVar10,(lsVec3 *)(tAB.row + 3));
  MVar7 = 0;
  if (bVar2) {
...

{
  ...
  lVar10 = (longdouble)McdSphereGetRadius(uVar7);
                    /* Unresolved local var: float __result@[???] */
  lVar11 = (longdouble)1.0 / (SQRT((longdouble)2.0) + (longdouble)1.0);
  fVar1 = (float)((lVar11 + lVar11) * lVar10);
  pfVar8 = (float *)McdBoxGetRadii(uVar6);
  ext1[0] = fVar1;
  ext1[1] = fVar1;
  ext1[2] = fVar1;
  iVar9 = MovingBoxBoxIntersect(pfVar8,plVar4,plVar2,ext1,plVar5,plVar3,maxTime,&T,&P);
  if (iVar9 == 0) {
    result->time = maxTime;
  }
  else {
