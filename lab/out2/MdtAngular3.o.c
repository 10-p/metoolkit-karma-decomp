...

{
  ...
  pMVar1 = (MdtAngular3ID)BaseConstraintCreate(w);
  pMVar3 = (MdtAngular3ID)0x0;
  if (pMVar1 != (MdtAngular3ID)0x0) {
    (pMVar1->head).tag = 0xc;
    (pMVar1->head).bclFunction = (MdtBclAddConstraintFn)&MdtBclAddAngular3;
...

{
  ...
  MdtConstraintSetAxis((iVar1 != 0xc) - 1 & (uint)c,tm[0][0],tm[0][1],tm[0][2]);
  return;
}
