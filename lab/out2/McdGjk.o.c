...

{
  ...
  lVar2 = (longdouble)McdGjkFatness(pMVar1);
  c->fat1 = (float)lVar2;
  lVar2 = (longdouble)McdGjkFatness(c->ins2);
  c->separation = 1.0;
  c->fat2 = (float)lVar2;
  c->normal[0] = 0.0;
  c->normal[1] = 1.0;
...

{
  ...
  puVar3 = (undefined4 *)(*_McdGeometryInstanceGetAABB)(pMVar1);
  ...
  if (puVar3 != (undefined4 *)0x0) {
    ...
    lVar5 = (longdouble)McdGjkFatness(pMVar4);
    puVar3[9] = (float)lVar5;
    lVar5 = (longdouble)McdGjkFatness(puVar3[0xe]);
    *puVar3 = 0;
    puVar3[0xc] = 0x3f800000;
    puVar3[10] = (float)lVar5;
    puVar3[1] = 0x3f800000;
    puVar3[2] = 0;
    puVar3[3] = 0;
    puVar3[4] = 0;
...

{
  ...
  if (p->m_cachedData != (void *)0x0) {
    (*_McdGjkPenetrationDepth)(&pMVar1->frame->cachePool,p->m_cachedData,pMVar1,pMVar1);
  }
...

{
  ...
  neg_v[2] = s1[2];
  if (sign < 1) {
    McdGjkMaximumPoint(c->ins1,neg_v,s1,sign);
    pMVar4 = v;
  }
  else {
    McdGjkMaximumPoint(c->ins1,v,s1,sign);
    pMVar4 = neg_v;
  }
  McdGjkMaximumPoint(c->ins2,pMVar4,p->s2,sign);
  fVar1 = s1[0] - p->s2[0];
  p->w[0] = fVar1;
  fVar3 = s1[1] - p->s2[1];
  p->w[1] = fVar3;
...

{
  ...
  c->offset[2] = 0.0;
  do {
    uVar7 = McdGjkMain(c,s);
    uVar8 = uVar7;
    if (uVar7 == 1) {
      bVar6 = false;
      if (s->bits == 0xf) {
        fVar2 = s->delta[0xf][0];
        uVar10 = (ushort)(fVar2 < 1e-06) << 8 | (ushort)NAN(fVar2) << 10 |
      ...
      if (bVar6) {
        uVar8 = McdGjkPenetrationDepth(c,s,uVar8,uVar8);
        uVar7 = 1 - uVar8;
      }
    }
    if (uVar7 == 0) {
    ...
    if (iVar9 == 0) {
      McdGeometryInstanceUpdateAABB(c->ins1,0,0,uVar8);
...
LAB_00011521:
    ...
    fVar3 = fVar2;
    if (iVar9 != 3) {
      fVar3 = (fVar2 - axisLen) * ((float)iVar9 + 1.0) * 0.25;
    }
...

{
  ...
  i = 1;
  do {
    in_EAX = McdGeometryInstanceGetSlice
                       (c->ins1,c,(float)i * fVar8 + fVar1 + c->fat1,0x21,&size1,vspace,in_EAX,
                        in_EAX);
    iVar13 = j0;
    if (size1 != 0) {
...

{
  ...
  pMVar8 = p->model1;
  if ((char)((pMVar8->mInstance).mGeometry)->mRefCtAndID == '\x03') {
    pMVar7 = pMVar8;
    pMVar8 = p->model2;
    p = (McdModelPair *)0x0;
  }
    ...
    if ((char)((pMVar7->mInstance).mGeometry)->mRefCtAndID != '\x03') {
      if (c == (McdCache *)0x0) {
        cmem.ins2 = &pMVar7->mInstance;
        cmem.padding = pMVar7->mPadding + pMVar8->mPadding;
        cmem.ins1 = &pMVar8->mInstance;
        c = &cmem;
        lVar6 = (longdouble)McdGjkFatness(cmem.ins1);
        cmem.fat1 = (MeReal)lVar6;
        lVar6 = (longdouble)McdGjkFatness(cmem.ins2);
        cmem.separation = 1.0;
        cmem.fat2 = (MeReal)lVar6;
        cmem.normal[0] = 0.0;
        cmem.normal[1] = 1.0;
        cmem.normal[2] = 0.0;
        cmem.location[0] = 0.0;
  ...
  iVar4 = McdPlaneIntersectTest(pMVar7,pMVar8,result,p);
  return iVar4;
}
