...
| `MdtLOD` 517 | **W** (closed) | `local_6c.conArray = (MdtBaseConstraint **)local_6c.bodyCount` — an `int` byte-cursor carried in a POINTER field and read straight back as `(kd_iptr)local_6c.conArray`. int→pointer is a widening; the slot is eight bytes at LP64 and the value round-trips unchanged. ⚠ **The same function has two defects this diagnostic cannot see and that are still open** — see `STATE.md`: `*(int *)((kd_iptr)&pdataArray->bodyArray + cur)` reads a 4/8 field four bytes wide, and the cursor steps by `0x14`, which is `sizeof(MdtLODPartitionData)` at i386 and **half** of it at LP64. Neither truncates a cast, so neither is in the census. |
