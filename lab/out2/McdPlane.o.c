...

{
  ...
  pMVar1 = (McdPlaneID)(*_McdGeometryDeinit)(0x10,0x10);
  if (pMVar1 != (McdPlaneID)0x0) {
    McdGeometryInit(pMVar1,frame,3,extraout_ECX);
  }
  return pMVar1;
}
...

{
  ...
  iVar1 = McdGeometryInstanceGetTransformPtr(ins);
  ...
  if (*(float *)(iVar1 + 0x20) <= 0.9999995) {
    if (-0.9999995 <= *(float *)(iVar1 + 0x20)) {
      if (*(float *)(iVar1 + 0x24) <= 0.9999995) {
        if (-0.9999995 <= *(float *)(iVar1 + 0x24)) {
          if (*(float *)(iVar1 + 0x28) <= 0.9999995) {
            if (*(float *)(iVar1 + 0x28) < -0.9999995) {
              ins->min[2] = *(MeReal *)(iVar1 + 0x38);
            }
          }
          else {
            ins->max[2] = *(MeReal *)(iVar1 + 0x38);
          }
        }
        else {
          ins->min[1] = *(MeReal *)(iVar1 + 0x34);
        }
      }
      else {
        ins->max[1] = *(MeReal *)(iVar1 + 0x34);
      }
    }
    else {
      ins->min[0] = *(MeReal *)(iVar1 + 0x30);
    }
  }
  else {
    ins->max[0] = *(MeReal *)(iVar1 + 0x30);
  }
...

{
  ...
  int iVar3;
                    ...
                    /* Unresolved local var: lsTransform * tm@[DW_OP_reg2(EDX)]
                       ...
                       Unresolved local var: MeReal cosTolAngle@[???] */
  iVar3 = McdGeometryInstanceGetTransformPtr(ins);
  if (inDir[2] * *(float *)(iVar3 + 0x28) +
      inDir[1] * *(float *)(iVar3 + 0x24) + *inDir * *(float *)(iVar3 + 0x20) < 0.9999995) {
    fVar1 = inDir[1];
    fVar2 = inDir[2];
    ...
    *outPoint = *(MeReal *)(iVar3 + 0x30);
    outPoint[1] = *(MeReal *)(iVar3 + 0x34);
    outPoint[2] = *(MeReal *)(iVar3 + 0x38);
  }
  return;
}
