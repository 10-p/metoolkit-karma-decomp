...
INC="$KD_MT_INC"
LIB="$KD_MT_LIB"
SRC="${KD_SRC:-${KD_OUT:-$KD_OUT_SRC}/allobj/IxCylinderCylinder.c}"
...
python3 "$KD_ROOT/tools/mk_trace_obj.py" "$SRC" "$MANG" > "$W/obj.c"

rm -rf "$W/b"; cp -a "$BUILD" "$W/b"
CF="-m32 -O2 -fno-pic -fno-strict-aliasing -std=gnu99 -w -Wno-int-conversion"
CF="$CF -Wno-incompatible-pointer-types -DLINUX -I$KD_MD_INC -I$INC -I$INC/McdCommon"
...
gcc -m32 -O2 -DLINUX -no-pie $IF -o "$W/dt" "$KD_T_STD/difftest_pair.c" \
