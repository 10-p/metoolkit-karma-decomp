...
void keaFunctions_Vanilla::calculateConstraintAndResultantForces
               (MdtKeaBody **blist,MdtKeaForcePair *cforcePairs,MdtKeaJBlock (*Jstore) [2],
               int (*Jbody) [2],MeReal *lambda,int (*bl2body) [8],int (*bl2cbody) [8],int *jlen,
               int num_rows_exc_padding,int num_rows_inc_padding,int num_constraints,int num_bodies)

{
  ...
  Lambda = (MeReal *)bl2body;
  J = (MdtKeaJBlock *)Jbody;
  strip = 0;
  while( true ) {
    iVar1 = num_rows_inc_padding;
    if (num_rows_inc_padding < 0) {
      iVar1 = num_rows_inc_padding + 3;
    }
    ...
    block = 0;
    iVar1 = *(int *)num_rows_exc_padding;
    piVar2 = *bl2cbody;
    piVar3 = jlen;
    jstrip = J;
    if (iVar1 != 0) {
      do {
        if (*piVar2 != -1) {
          Multiply4VectorWith46Matrix
                    ((MeReal *)((int)(cforcePairs->primary_body).force[*piVar2] + 0x10),
                     (MdtKeaForce *)((*Jstore)[0].col + *piVar3 * 2),Lambda,jstrip);
          iVar1 = *(int *)num_rows_exc_padding;
        }
        piVar2 = piVar2 + 1;
        block = block + 1;
    ...
    bl2cbody = bl2cbody + 1;
    jlen = jlen + 8;
    num_rows_exc_padding = num_rows_exc_padding + 4;
    strip = strip + 1;
  }
  return;
}
