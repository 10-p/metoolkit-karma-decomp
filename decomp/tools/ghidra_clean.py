...
def _compat_variadic_arity():
    ...
    path = os.path.join(kd_paths.MD_INC, 'kd_compat.h')
    out = {}
    try:
...
def fix_incomplete_pointer_arithmetic(text, diag, ctx):
    ...
    seen = ''
    for d in (ctx.include_dir, kd_paths.MD_INC):
...
def _typedb(path=None):
    ...
    if 'text' not in _TYPEDB_TEXT:
        p = path or os.path.join(kd_paths.MD_INC, 'kd_types.h')
        try:
...
def main():
    ...
    # ⚠ THE PROVENANCE LINE IS REPO-RELATIVE ON PURPOSE. It used to record the
    # absolute path the object happened to be read from, which put one
    # developer's home directory into all 145 checked-in sources and made the
    # generated tree depend on WHERE it was generated. Relative to the repo
    # root, two people regenerating from the same lab now get the same bytes.
    # It is a comment, so this cannot move an object: the i386 acceptance test
    # still reads 145/145 byte-identical across the change, and that is the
    # proof rather than the argument.
    obj_note = os.path.abspath(args.object)
    if obj_note.startswith(kd_paths.REPO + os.sep):
        obj_note = os.path.relpath(obj_note, kd_paths.REPO)

    with open(args.output, 'w') as f:
        f.write('/* Generated by decomp/tools/ghidra_clean.py — do not edit by hand.\n'
                f' * source object: {obj_note}\n'
                ' */\n')
