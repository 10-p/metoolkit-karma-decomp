...
| `McdInteractions` 104 | ✅ **CLOSED** (`fix_narrow_loads` rule F — the callee's DECLARED return type) | `(code *)*puVar4 != 0` — a four-byte read of a function pointer. ⚠ **The oracle declined and this file said "unresolved" for it**: amd64 `McdHello` uses **both** `mov (%rax),%rax` and `mov (%rax),%eax` at displacement 0. Both are right — the narrow one is `McdGeometry::mRefCtAndID`, a genuine `MeU32` at offset 0 of a *different* object. A displacement is not an object. The HEADER is: `McdFrame.h` declares `McdInteractions *MEAPI McdFrameworkGetInteractions(...)` and offset 0 of that struct is `McdHelloFn helloFn`, 4 at i386 and 8 at LP64. The call two lines below reads the same word at eight bytes. |
| `MdtLOD` 224 | ✅ **CLOSED** (`fix_frame_slots` slot-member rule — the name shape and the bare deref) | `*(MdtBaseConstraint **)local_6c.rowCount` where `local_6c.rowCount = (kd_iptr)(ppMVar18 + 1)` — a stack ADDRESS in a four-byte field named for a count, then dereferenced. `local_6c` is a fabricated aggregate: the real array of that type is `pdataArray`, a separate `alloca`. The rule already existed and missed it twice — Ghidra spells this one `local_6c` rather than `auStack_78`, and the deref has no arithmetic on it so there was no `(` for the use-pattern to find. |
| `MdtLOD` 517 | **W** (closed) | `local_6c.conArray = (MdtBaseConstraint **)local_6c.bodyCount` — an `int` byte-cursor carried in a POINTER field and read straight back as `(kd_iptr)local_6c.conArray`. int→pointer is a widening; the slot is eight bytes at LP64 and the value round-trips unchanged. ⚠ **The same function has two LP64 defects this diagnostic cannot see, and they are OUT OF SCOPE, not open** — `*(int *)((kd_iptr)&pdataArray->bodyArray + cur)` reads a 4/8 field four bytes wide, and the cursor steps by `0x14`, which is `sizeof(MdtLODPartitionData)` at i386 and **half** of it at LP64. `MdtLODLastPartition` is the only symbol the engine imports from this object and its one call site is guarded by `rowCount > maxMatrixSize` with `maxMatrixSize = 0x7ffffffc`, nothing anywhere lowering it: a breakpoint fires **zero** times in a 235 s match. The engine links this code and never enters it. ★ If anything ever calls `MdtWorldSetMaxMatrixSize` with a real bound, both rows are defects again. |
| `IxCylinderTriList` 148 | ✅ **CLOSED** (`fix_frame_slots` — the narrowing store cast) | `kd_slot_MStack_26c_flags = (McdTriangleFlags)result->normal` — the slot had already been promoted to `kd_iptr` by this rule and the **store into it was still cut in half** by an enum cast, which is four bytes at both widths. Wide storage, narrow value: the repair was only half-applied and had been shipping that way. |
...
| `IxSphereTriList` ×3 | ✅ **CLOSED — THEY ARE NOT DEFECTS, THE PASS WAS.** Quarantine lifted; `QUARANTINE` is now empty. `(float *)(pvVar21 + 0x30)` is byte 48 of a **4×4 MATRIX**, not of a geometry: `pvVar21` was reassigned by `McdModelGetTransformPtr` two lines earlier, and `evidence()` is whole-function. Three more the write-up had never named were worse — `((int)aiStackY_180 + 0x10)`, a **stack argument area**, was being re-spelled as `offsetof(McdSphere, mRadius)`, which is 32 at LP64 and moves an argument word. Both are now positive disqualifications in `base_disqualified()`. ★ And the "pipeline hazard" was a REGEX THAT KNEW ONE SPELLING, not all-or-nothing acceptance — see `STATE.md`; fixing it closed a live heap under-allocation in `IxBoxTriList` and `IxSphylPrimitives` too. |
