...
McdGeometry * McdConvexMeshCreate(McdFramework *frame,McdConvexHull *poly,MeReal fatness)

{
  ...
  pMVar1 = (*_McdComputeHull)(0x40,0x10);
  ...
  pMVar1[2].frame = (McdFrameworkID)fatness;
  MeBoundingSphereCalc2
...

{
  ...
  if (iVar1 != 0) {
                    /* Unresolved local var: McdConvexMesh * c@[DW_OP_reg3(EBX)]
                       Unresolved local var: McdGeometry * g@[???] */
    pMVar2 = (*_McdComputeHull)(0x40,0x10);
